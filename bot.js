...
const discord_token = "NDUyOTMwOTU2OTM3MDAzMDA4.DfXglQ.qXLNlevRbDT1By1VoLfEfL5hSaY";
client.login("NDUyOTMwOTU2OTM3MDAzMDA4.DfXglQ.qXLNlevRbDT1By1VoLfEfL5hSaY");
/*
////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\
////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\
////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\
...
client.on('message', function(message) {
	...
	if (mess.startsWith(prefix + 'play')) {
		if (!message.member.voiceChannel) return message.channel.send(':no_entry: || **__You must be in Voice channel__**');
		// if user is not insert the URL or song title
		if (args.length == 0) {
			let play_info = new Discord.RichEmbed()
				.setAuthor(client.user.username, client.user.avatarURL)
				.setFooter('Requested by: ' + message.author.tag)
				.setDescription('**Insert the link or name of the song**')
		...
		if (queue.length > 0 || isPlaying) {
			...
			getID(args, function(id) {
				...
				fetchVideoInfo(id, function(err, videoInfo) {
					...
					let play_info = new Discord.RichEmbed()
						.setAuthor(client.user.username, client.user.avatarURL)
						.addField('||**___was turned on___**', `**${videoInfo.title}
							  **`)
						.setColor("RANDOM")
                        .addField(`__By__: ${message.author.username}`, `**Discord.js**`)
						.setThumbnail(videoInfo.thumbnailUrl)
		...
		if (!message.member.voiceChannel) return message.channel.send(':no_entry: || **__You must be in Voice channel__**');
		// console.log(args)
		if (args > 100) return message.channel.send('1 - 100 || **__No more, no less__**')
		if (args < 1) return message.channel.send('1 - 100 || **__No more, no less__**')
		dispatcher.setVolume(1 * args / 50);
		message.channel.sendMessage(`**__ ${dispatcher.volume*50}% Volume level __**`);
	}
		...
		if (!message.member.voiceChannel) return message.channel.send(':no_entry: || **__You must be in Voice channel__**');
		if (isPlaying == false) return message.channel.send(':anger: || **__Stopped__**');
