...
client.on('message', async message => {
  ...
  let args = messageArray.slice(1);
    if(message.content.startsWith(prefix + "fortnite")) {
        ...
        let gamemode = args[1];
        if(gamemode != 'solo' && gamemode != 'duo' && gamemode != 'squad' && gamemode != 'lifetime') return message.reply('How to use : ${prefix}fortnite username mode platform ```Example: +fortnite Ninja solo pc```');
